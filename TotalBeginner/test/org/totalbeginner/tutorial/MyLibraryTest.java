...
public class MyLibraryTest extends TestCase {
	...
	public void testAddBooks() {
		...
		public void testCheckOut() {
			...
			assertTrue("Book did not check correctly", m1.checkOut(b1,p1));
			
			assertEquals("Fred", b1.getPerson().getName());	
			
			assertFalse("Book was already checked out", m1.checkOut(b1,p2));
			
			assertTrue("Book checked in failed", m1.CheckIn(b1));	
			
			assertFalse("Book was already checked in", m1.CheckIn(b1));
			
			assertFalse("Book was never checked", m1.CheckIn(b2));
